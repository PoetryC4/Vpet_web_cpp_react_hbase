...
Drink DrinkController::getById(const std::string &drinkId) {
...
MyPageResult<Drink> DrinkController::page(const int &page, const int &pageSize) {
    std::shared_ptr<TSocket> socket(new TSocket(THRIFT_IP, THRIFT_PORT));
    std::shared_ptr<TTransport> transport(new TBufferedTransport(socket));
    std::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
...
long DrinkController::save(const DrinkAddRequest &drinkAddRequest) {
    std::shared_ptr<TSocket> socket(new TSocket(THRIFT_IP, THRIFT_PORT));
    std::shared_ptr<TTransport> transport(new TBufferedTransport(socket));
    std::shared_ptr<TProtocol> protocol(new TBinaryProtocol(transport));
    long resId = -1;
