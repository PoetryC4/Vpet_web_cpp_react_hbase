...
import {animationGap, debugSleep} from "@/components/Entity/petConstants";
import {
  animationTimestamp,
  durDebug,
  durShutdown,
  ...
  petLeftMargin,
  petTopMargin,
  setAnimationTimestamp,
  setItemTransformScale,
  setPetLeftMargin,
  setPetTopMargin
...
const imagesPool = {}
const itemImagePool = {}
const tryLoadImages = (path: string) => {
  if (imagesPool[path] !== undefined) {
    return
  }
  console.log(path)
  console.log(imageList[path])
  if (process.env.NODE_ENV === 'development') {
    imagesPool[path] = imageList[path].reduce((acc, currentValue, index) => {
      acc[currentValue] = require("../../assets/images/" + path + '/' + currentValue);
      return acc;
    }, {})
  } else {
    imagesPool[path] = imageList[path].reduce((acc, currentValue, index) => {
      acc[currentValue] = "assets/images/" + path + '/' + currentValue
      return acc;
    }, {})
  }
}
const tryLoadSingleImage = (itemPath: string) => {
  if (itemImagePool[itemPath] !== undefined) {
    return
  }
  if (process.env.NODE_ENV === 'development') {
    itemImagePool[itemPath] = require("../../assets/images/" + itemPath);
  } else {
    itemImagePool[itemPath] = "assets/images/" + itemPath
  }
}
...
// 奇怪的参数
const itemLayShift = [-398, 0]
const backLayShift = [0, -19]
// 物品的变换参数
...
export async function petAnimation(path: string, isDebug: boolean) {
  if ((!isLaunched.value && !durShutdown.value) || (durDebug.value && !isDebug)) return
  tryLoadImages(path)
  //console.log(path)
  ...
  for (let imgName of allBackLayImgs) {
    ...
    hooksToBeExported.trySetPetBackLayImgUrl(imagesPool[path][imgName])
...
export async function petAnimation3layers(charPath: string, itemPath: string, startIndex: number, itemTransformIndex: number, isDebug: boolean) {
  ...
  // 180是试出来的值
  tryLoadImages(charPath + '/back_lay')
  tryLoadImages(charPath + '/front_lay')
  tryLoadSingleImage(itemPath)
  ...
  hooksToBeExported.trySetPetFrontLayStyle({
    ...
    marginLeft: petLeftMargin.value + backLayShift[0] + 'px',
    marginTop: petTopMargin.value + backLayShift[1] + 'px',
    position: 'absolute'
  })
  hooksToBeExported.trySetPetItemLayStyle({
    ...(hooksToBeExported.tryGetPetItemLayStyle()),
    marginLeft: petLeftMargin.value + itemLayShift[0] + 'px',
    marginTop: petTopMargin.value + itemLayShift[1] + 'px',
    position: 'absolute'
  })
  ...
  for (let i = 0; i < allBackLayImgs.length; i++) {
    ...
    // 另一个动画开始后，当前动画结束
    if (thisAnimationTimestamp !== animationTimestamp.value) break
    hooksToBeExported.trySetPetBackLayImgUrl(imagesPool[charPath + '/back_lay'][allBackLayImgs[i]])
    // 处理前置层和物品图层
    if (i >= startIndex && i < startIndex + allFrontLayImgs.length) {
      ...
      hooksToBeExported.trySetPetFrontLayImgUrl(imagesPool[charPath + '/front_lay'][allFrontLayImgs[i - startIndex]])
      ...
      hooksToBeExported.trySetPetItemLayImgUrl(itemImagePool[itemPath])
    } else {
    ...
    await sleep(animationGap)
    console.log(hooksToBeExported.tryGetPetFrontLayStyle())
    console.log(hooksToBeExported.tryGetPetBackLayStyle())
  }
  hooksToBeExported.trySetPetFrontLayStyle({...(hooksToBeExported.tryGetPetFrontLayStyle()), display: 'none'})
  hooksToBeExported.trySetPetItemLayStyle({...(hooksToBeExported.tryGetPetItemLayStyle()), display: 'none'})
  if (isDebug) {
    ...
    case 'r': {
      while (petLeftMargin.value + imgSize.value / 2 < target) {
        ...
        for (let i = 0; i < allBackLayImgs.length; i++) {
          ...
          hooksToBeExported.trySetPetBackLayImgUrl(imagesPool[path][allBackLayImgs[i]])
          await sleep(animationGap)
        }
      }
      break
