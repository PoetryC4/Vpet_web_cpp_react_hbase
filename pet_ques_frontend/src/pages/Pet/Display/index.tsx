...
import {imgSize, setDurDebug, setDurSwitch, setPreLaunched} from "@/components/Entity/petStateProperty";
...
const PetDisplay: React.FC = () => {
  ...
  const [petMoney,setPetMoney,refPetMoney]=useState(0)
  ...
  async function petDragMove(event) {
    if (!isLaunched.value || !isDragged.value) return
  ...
  async function petListener() {
    ...
    if (curAnimation.value in interactionNames) {
      ...
      setPetMoney(refPetMoney.current + ((petValueChange[curAnimation.value].moneyChange(petExp, petWorkPerformance.value)) || 0))
      return
    }
  ...
  const beginChat = async () => {
    ...
    try {
      ...
      setChatPopoverStyle({
        ...chatPopoverStyle,
        marginLeft: petLeftMargin.value,
        marginTop: petTopMargin.value + chatPopoverTopShift,
        display: 'block'
      })
  ...
  return (
    <div id={'petDisplayView'}>
      <Menu
        ...
        items={menuItems}
      />
      <Popover placement={popoverPlacement} title={chatResponseDisplay.title} content={chatResponseDisplay.content}
